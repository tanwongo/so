...
var Janimated = {
    ...
    lineAnimate: function (attr, id, status, cls, isload) {
        var temp = "<div class='" + cls + "' id='lineAnimate'></div>";
        if (status) {
            $("[" + attr + "=" + id + "]").append(temp);
     ...
     * @param cls  动画样式
     */
    downAnimate: function (id, status, cls) {
        var temp = "<div style='width:1000px;height:30px;background:#000;display:none' id='aaaa'></div>";
        $(id).append(temp);
        if (status) {
    ...
    succAnimate: function (dom, status, cls, txt) {
        if (status) {
            var temp = "<div class=\"" + cls + "\"><b class=\"txt\">" + txt + "</b></div>";
            dom.append(temp);
            setTimeout(function () {
                dom.find("." + cls + "").remove();
            }, 2000);
